...
public class ValidarController extends HttpServlet {
	...
	List<Map<String,Object>> lista = ud.validar(user, clave);
	int x = lista.size();
	String mensaje = "";
	if(x == 1) {
		//sesion.setAttribute("usuario", lista.get(0).);
		for (Map<String, Object> map : lista) {
		        sesion.setAttribute("idu", map.get("idusuario"));
		        sesion.setAttribute("user", map.get("user"));
		        sesion.setAttribute("rol", map.get("rol"));
